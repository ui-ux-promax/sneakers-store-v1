...
> 1. **preview build зелёный**;
> 2. в Vercel (preview) заданы env: `RESEND_API_KEY`, `EMAIL_FROM_TRANSACTIONAL`, `EMAIL_FROM_NEWSLETTER`,
>    `EMAIL_REPLY_TO` (опц.), `RESEND_AUDIENCE_ID`, `NEXT_PUBLIC_SITE_URL` (+ уже существующий `AUTH_SECRET`);
> 3. к preview-БД применён **`prisma db push`** (новые таблицы `EmailVerificationCode`, `Subscriber`);
> 4. домен `cloudd3r.eu.cc` верифицирован в Resend, `RESEND_AUDIENCE_ID` указывает на аудиторию **General**.
>
> Подготовь **реальный почтовый ящик** (письма уходят по-настоящему). Для одноразовых адресов удобно
> использовать `+`-алиасы (`ваша+стрид1@gmail.com`) — каждый = новый «пользователь».

### A. Регистрация → жёсткий gate
1. [ ] `/register` новым email+паролем → submit. Остаёшься на странице, поверх **появляется модалка «Подтвердите почту»** (не редиректит на `/`, **в шапке НЕ залогинен**).
2. [ ] На почту пришло письмо от **`no-reply@cloudd3r.eu.cc`** с 6-значным кодом (тема «Код подтверждения STRIDE»).
3. [ ] В модалке текст «Код отправлен на `e***@домен`» — email **замаскирован**.

### B. Неубираемость модалки (ключевое требование)
1. [ ] **Esc** не закрывает. Клик по затемнению **вне** окна не закрывает. Крестика нет.
2. [ ] **Перезагрузи страницу (F5)** при открытой модалке → модалка **снова на месте** (переживает reload).
3. [ ] Перейди руками на другой URL (напр. `/catalog`) → модалка **всё равно поверх** (гейт глобальный, в layout).
4. [ ] Через ~30 мин (TTL cookie) модалка пропадёт без верификации — но войти всё равно нельзя (см. D).

### C. Ввод кода — все ветки
1. [ ] **Неверный** код (6 цифр) → «Неверный код…», поле очищается, можно ввести снова.
2. [ ] **Авто-сабмит**: при вводе 6-й цифры запрос уходит сам (не нужно жать «Подтвердить»).
3. [ ] **5 неверных** попыток подряд → «Слишком много попыток…» (код залочен), нужен ресенд.
4. [ ] **«Отправить код снова»** → новое письмо, кнопка уходит в кулдаун `Отправить снова через 60с` (таймер тикает).
5. [ ] **Верный** код → модалка исчезает, **в шапке появляется залогиненное состояние** (auto-login через verified-ticket), редиректа-ошибки нет.
6. [ ] Старый код после ресенда/успеха больше не подходит (одноразовость): повторный ввод предыдущего → «истёк/неверно».

### D. Вход неверифицированного (password gate)
1. [ ] Зарегистрируй email, но **НЕ вводи код** (закрой вкладку). Открой `/login`, войди тем же email+паролем.
2. [ ] Вход **не пускает** — вместо входа поднимается **та же модалка верификации** + приходит новое письмо с кодом.
3. [ ] Введи верный код в этой модалке → теперь залогинен. (Подтверждает, что gate на входе работает и переиспользует модалку.)
4. [ ] Неверный **пароль** для существующего верифицированного юзера → обычная ошибка «Неверный email или пароль» (без утечки, что email существует).

### E. Google-вход (авто-verify)
1. [ ] Войди через **Google** новым google-аккаунтом → входит **сразу, без модалки и кода** (OAuth-почта уже проверена).
2. [ ] В БД у этого юзера `emailVerified` проставлен (можно косвенно: повторный вход работает без гейта).

### F. Newsletter (рассылка)
1. [ ] Футер → ввод email → «Подписаться» → кнопка «Готово», приходит welcome-письмо от **`hello@cloudd3r.eu.cc`**.
2. [ ] В Resend → Audience **General** появился контакт с этим email (синк сработал).
3. [ ] Повторная подписка тем же email → «Вы с нами» (`alreadySubscribed`), **второго welcome НЕ шлёт**.
4. [ ] В welcome-письме ссылка **«Отписаться»** → ведёт на `/unsubscribe?token=…` → «Вы отписались».
5. [ ] После отписки в Resend Audience контакт помечен `unsubscribed`. Повторная подписка тем же email → **реактивация** (welcome снова приходит).
6. [ ] Битый/обрезанный `?token=` на `/unsubscribe` → «Ссылка недействительна» (HMAC отверг подделку).

### G. Регрессия
1. [ ] Существующие флоу не сломаны: вход **верифицированного** юзера паролем, корзина/вишлист сливаются при входе (merge как раньше).
2. [ ] Защищённые маршруты (`/profile`, `/checkout`, `/orders`) по-прежнему требуют сессию.
3. [ ] CI: `npm test` (212 зелёных) + e2e (`e2e.yml`) проходят; сток размера 42 не просел (бюджет заказов — [[e2e-size42-stock-budget]]).

### На что смотреть при сбое
- **Письмо не приходит** → проверь `RESEND_API_KEY` (Full access, не только Sending), верификацию домена в Resend, лог `email_send_failed`/`email_not_configured`. В dev без ключа код печатается в лог (`verification_code_dev`).
- **Модалка не появляется после регистрации** → cookie `pending_verification` не ставится (проверь `AUTH_SECRET` задан) ИЛИ `VerificationGateHost` не в layout; проверь, что `registerUser` вернул `{ok:true,needsVerification:true}` и форма сделала `router.refresh()`.
- **Модалка не исчезает после верного кода** → `signIn('verified-ticket')` не поставил сессию (next-auth `5.0.0-beta.25` + Next 15): проверь, что провайдер `verified-ticket` есть в `auth.config.ts` и `verifyTicket` валиден; в логе `verified_ticket_signin_failed`. Fallback — войти паролём (после verify пароль работает).
- **Рантайм падает на `prisma.subscriber`/`emailVerificationCode`** → к БД не применён `db push` (нет таблиц). Прогони в CI/Vercel ([[neon-schema-not-auto-applied]]).
- **Newsletter синк не доходит до Resend** → `RESEND_AUDIENCE_ID` пуст/неверен или ключ без Contacts-прав; подписка при этом НЕ падает (best-effort), `resendContactId` остаётся null, лог `audience_sync_failed`.
- **Google-юзер упёрся в gate** → `events.signIn` не проставил `emailVerified` (проверь, что блок `account?.provider==='google'` в `auth.ts` не потёрт).
- **Письма с битым `from`** → в Vercel env значение `EMAIL_FROM_*` записано **с кавычками** (надо без) — Resend получает кавычки в имени отправителя.
