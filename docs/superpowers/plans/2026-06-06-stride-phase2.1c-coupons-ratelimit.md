...
> Гоняется на **preview-деплое** ветки (не локально — Neon-латентность + локальный db push заблокирован, P1017/P4). Сначала убедись, что **preview build зелёный** в Vercel (build применяет `db push` → таблица `Coupon` + колонки `Order.discountAmount`/`couponCode` в preview-Neon-ветке, P7). **Важно:** seed на preview НЕ запускается автоматически (build делает только `db push`), поэтому таблица `Coupon` будет **пустой** — перед проверкой завести купоны в preview-Neon-ветке вручную (Neon SQL editor / Prisma Studio на preview-URL), либо разово прогнать `npm run prisma:seed` с `POSTGRES_URL` от preview-ветки.

### Шаг 0 — предусловия
- [ ] Vercel → деплой ветки `feat/phase2.1c-coupons` → build **зелёный** (в логе `prisma db push` / `in sync`; появились `Coupon`, `Order.discountAmount`, `Order.couponCode`).
- [ ] В preview-Neon-ветке заведены купоны: минимум `STRIDE10` (percent=10, active=true, expiresAt=null) и `EXPIRED` (active=true, expiresAt в прошлом). Пример SQL: `INSERT INTO "Coupon"(id,code,percent,active,"createdAt") VALUES (gen_random_uuid(),'STRIDE10',10,true,now()),(gen_random_uuid(),'EXPIRED',50,true,'2020-01-01');`
- [ ] (Для проверки rate-limit, секция D) в Vercel Preview заданы `KV_REST_API_URL`/`KV_REST_API_TOKEN`. Без них лимитер **fail-open** — секция D не сработает (это ожидаемо, не баг).
- [ ] Войти на preview под своим аккаунтом (auth в проде с P2.0).

### A. Промокод — основной путь
1. [ ] Товар → корзина → `/checkout`.
2. [ ] В блоке «Итого» ввести **STRIDE10** → «Применить» → появляется бейдж **«Промокод STRIDE10 (10%)»**, строка **«Скидка −N ₽»**, «Итого» уменьшилось ровно на 10% от суммы товаров.
3. [ ] Доставка НЕ подешевела из-за купона: порог бесплатной доставки считается от суммы товаров **до** скидки (если был «Бесплатно» — остаётся; если «по тарифу» — не переключился в бесплатно из-за скидки).
4. [ ] Нажать **× (Убрать промокод)** → скидка исчезла, «Итого» вернулось.
5. [ ] Снова применить STRIDE10 → оформить **COD**-заказ → на `/orders/<N>` видна строка **«Скидка (STRIDE10)»** и корректное «Итого» (со скидкой).
6. [ ] Профиль → «Мои заказы» → заказ виден.

### B. Промокод — негативные сценарии
1. [ ] Ввести **EXPIRED** → «Применить» → ошибка **«Срок действия промокода истёк»**, скидка не применилась, «Итого» без изменений.
2. [ ] Ввести мусор (напр. `NOPE123`) → ошибка **«Промокод недействителен»**.
3. [ ] Ввести в нижнем регистре `stride10` → применяется (нормализация trim+UPPERCASE).

### C. Скидка уходит в оплату (online)
1. [ ] Применить STRIDE10 + выбрать **«Картой онлайн»** → «Оформить заказ →».
2. [ ] На контракте ЮKassa **сумма = итог со скидкой** (itemsTotal − discount + доставка), НЕ полная сумма. (Сверить с «Итого» на чекауте.)

### D. Rate-limit входа (только при настроенном Upstash)
1. [ ] Выйти. На `/login` **5 раз** ввести верный email + НЕВЕРНЫЙ пароль (одинаковый email, тот же браузер/IP).
2. [ ] На **6-й** попытке вход отклоняется даже с **ВЕРНЫМ** паролем (сработал sliding-window 5/5мин) — ведёт себя как неверные данные.
3. [ ] Через ~5 минут окно сбрасывается, верный пароль снова пускает.
4. [ ] Без `KV_REST_API_*` (fail-open) — троттлинга нет, верный пароль пускает сразу (это норма для dev/preview без Upstash).

### E. Регрессия (без купона / P2.1a-b)
1. [ ] Оформить COD-заказ **без** купона → на `/orders/<N>` строки «Скидка» НЕТ, «Итого» = товары + доставка.
2. [ ] Онлайн-оплата без купона не сломана (редирект на ЮKassa, сумма полная).
...
- «Промокод недействителен» на ЗАВЕДОМО валидный код → купона нет в preview-Neon-ветке (seed не прогонялся — см. Шаг 0) или код в БД не в верхнем регистре.
- Скидка на чекауте есть, но при оформлении ошибка → купон протух между preview-сессией и сабмитом (повторная валидация в `placeOrder`) — обнови страницу.
- Rate-limit «не срабатывает» → `KV_REST_API_*` не заданы в Vercel Preview (fail-open) — это ожидаемо.
